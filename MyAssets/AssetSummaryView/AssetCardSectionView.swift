...
struct AssetCardSectionView: View {
    ...
    var body: some View {
        VStack {
            ...
            TabMenuView(tabs: ["지난달 결제", "이번달 결제", "다음날 결제"],
                    ...
                    ForEach(0...2, id: \.self) { i in
                        VStack {
                            ForEach(assetData) { data in
                                HStack {
                                    Text(data.title).font(.title).foregroundColor(.secondary)
                                    Spacer()
                                    Text(data.creditCardAmounts![i].amount).font(.title2).foregroundColor(.primary)
                                }
                                Divider()
                            }
                        }
